    ...
    if (rolSeleccionado) {
      ...
      db.rol.clear().then(() => {
        ...
        db.rol.add(rolSeleccionado).then(() => {
          toast.success(`Rol: ${rolSeleccionado.nombre}. Guardado Exitosamente.`);
        }).catch((error) => {
          toast.error(`Error al guardar el rol seleccionado en la base de datos. ${error}`);
        });
      }).catch((error) => {
        toast.error(`Error al borrar los registros de la tabla rol. ${error}`);
      });
    }
  };
